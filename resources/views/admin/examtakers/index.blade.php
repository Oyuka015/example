...
<section class="admin_register_section">
      ...
      <div class="card card-body">
        <form method="POST" id="examtakers-search-form" class="form-horizontal form-bordered smart-form" action="javascript:;" enctype="multipart/form-data">
          <div class="row">
            ...
            <div class="col-md-4">
              <div style="padding:5px">
                <input type="text" id="register" name="register" placeholder="{{trans('display.register')}}">
              </div>
            </div>
            <div class="col-md-4">
              <div style="padding:5px">
                <input type="text" id="phone" name="phone" placeholder="{{trans('display.phone')}}">
              </div>
            </div>
            <div class="col-md-4">
              <div style="padding:5px">
  ...
  <!-- collapse end -->
  <div style="margin-bottom: 10px">
    <button type="button" class="link-1" id="examtakers-add" data-toggle="modal" data-target="#examtakers-add-modal" style="border-color:white">{{trans('display.add_new')}}</button>
  </div>
  ...
  <table cellpadding="0" cellspacing="0" border="0" id="examtakers-table">
    <thead class="tbl-header">
      <tr>
        ...
        <th style="width:50%">{{trans('display.family_name')}}</th>
        <th style="width:50%">{{trans('display.surname')}}</th>
        <th style="width:50%">{{trans('display.lastname')}}</th>
        <th style="width:50%">{{trans('display.register')}}</th>
  ...
  <div class="modal fade" id="examtakers-add-modal" tabindex="-1" role="dialog" aria-labelledby="examtakers-add-modalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="examtakers-add-modalLabel">{{trans('display.add_new')}}</h4>
  ...
  <div class="modal fade" id="examtakers-edit-modal" tabindex="-1" role="dialog" aria-labelledby="examtakers-edit-modalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="examtakers-edit-modalLabel">{{trans('display.edit')}}</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
...
<script>
  var examtakersTable = $('#examtakers-table').DataTable( {
      ...
      ajax: {
          ...
          data: function ( d ) {
              ...
              d.user_name = $('#examtakers-search-form input[id="user_name"]').val();
              d.surname = $('#examtakers-search-form input[id="surname"]').val();
              d.lastname = $('#examtakers-search-form input[id="lastname"]').val();
              d.register = $('#examtakers-search-form input[id="register"]').val();
              d.phone = $('#examtakers-search-form input[id="phone"]').val();
          }
      },
      columns: [
        { 
  ...
  //add role
  $("#examtakers-add").on('click', function(){
    $.get( '/admin/examtakers/create', function( data ) {
      $('#examtakers-add-modal .modal-body').html(data);
      $('#examtakers-add-form').validate({
        ignore: [],
        ...
        submitHandler: function(form) {
          ...
          $.ajax({
            ...
            success: function(response) {
                $('#examtakers-add-modal').modal('hide');
  ...
  //edit role
  $('#examtakers-table tbody').on( 'click', 'tr td a.examtakers-edit', function () {
    var examtakersId = $(this).data('examtakersid');

    $.get( '/admin/examtakers/'+examtakersId+'/edit', function( data ) {
      $("#examtakers-edit-modal").modal('show');
      $('#examtakers-edit-modal .modal-body').html(data);
      $('#examtakers-edit-form').validate({
        ignore: [],
        highlight:function(element) {
            $(element).parents('.form-group').addClass('has-error has-feedback');
        },
        ...
        submitHandler: function(form) {
          ...
          $.ajax({
            ...
            success: function(response) {
                $('#examtakers-edit-modal').modal('hide');
                $('.form-sub-heading').html(response).fadeIn().delay(5000).fadeOut();
                examtakersTable.draw();
            },
            error: function (xhr, textStatus, error) {
